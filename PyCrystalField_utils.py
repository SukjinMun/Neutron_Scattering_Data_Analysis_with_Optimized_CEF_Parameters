...
class LS_Ligands:
    ...
    def TMPointChargeModel(self, l=2, symequiv=None, LigandCharge= -2, IonCharge=2,
                        printB = True, suppressminusm = False):
...
class LS_CFLevels:
    """For calculating and fitting crystal field levels for an ion"""
    def __init__(self, StevensOperators, Parameters, L,S, SpinOrbitCoupling, SpinSpin, MoleField):
        """add Stevens operators to make a single hamiltonian matrix.
        ...
        self.H_SOC = Lx*Sx + Ly*Sy + Lz*Sz
        LdotS = self.H_SOC.O*1.0
        self.H_SSC = Lx*Sx*Lx*Sx + Ly*Sy*Ly*Sy + Lz*Sz*Lz*Sz + Lx*Sx*Ly*Sy + Lx*Sx*Lz*Sz + Ly*Sy*Lx*Sx + Ly*Sy*Lz*Sz + Lz*Sz*Lx*Sx + Lz*Sz*Ly*Sy + 0.5 * Lx*Sx + Ly*Sy + Lz*Sz - (1/3)*self.L*(self.L+1)*self.S*(self.S+1)
        LdotSsquare = self.H_SSC.O*1.0
        self.H_mol = Sz
        Smol = self.H_mol.O*1.0
        
        if np.sum(LdotS.imag) == 0: LdotS = LdotS.real
        self.H_SOC.O = SpinOrbitCoupling*LdotS - SpinSpin*LdotSsquare + MoleField*Smol
    ...
    @classmethod
    def Bdict(cls,  Bdict, L, S, SpinOrbitCoupling, SpinSpin, MoleField):
        ...
        newcls = cls(Stev_O, Parameters, L, S, SpinOrbitCoupling, SpinSpin, MoleField)
        return newcls

    @classmethod
    def Hamiltonian(cls, CEF_Hamil, SOC_Hamil, L, S):
        newcls = cls([0,0],[0,0], L, S, 0, 0, 0) # Create empty class so we can just define Hamiltonian
